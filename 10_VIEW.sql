/*
        * VIEW (뷰)
          : SELECT문을 통해 얻어진 결과물을 저장해둘 수 있는 객체
            => 자주 사용되는 쿼리문을 저장해두면 매번 다시 해당 쿼리문을 기술할 필요 없음
            
            
        : 임시테이블과 같은 존재 (실제 데이터를 저장하는게 아니라, 논리적으로만 저장되어있음)
*/
...
--> TEXT 컬럼에 저장된 서브쿼리 정보가 있음!
 ...
 AS 
 (
    SELECT EMP_ID, EMP_NAME, DEPT_TITLE, SALARY, NATIONAL_NAME, BONUS
    FROM EMPLOYEE 
        JOIN DEPARTMENT ON DEPT_ID = DEPT_CODE
        JOIN LOCATION ON LOCAL_CODE = LOCATION_ID
        JOIN NATIONAL USING(NATIONAL_CODE ));
...
-- 위에서 조회한 쿼리문을 뷰에 저장 (VW_EMP_JOB) --> 뷰 생성
...
--> 오류 발생! 함수식 또는 연산식을 사용한 경우 별칭을 부여해줘야 뷰가 생성됨!
...
SELECT * FROM VW_EMP_JOB WHERE 근무연수 >= 20;
...
-- * VW_JOB 이라는 뷰를 사용하여 데이터 변경 (DML-UPDATE)
UPDATE VW_JOB SET JOB_NAME = '알바'
        WHERE JOB_CODE = 'J8';
 
-- * VW_JOB 뷰를 사용하여 데이터 삭제 (DML-DELETE)
...
/*
    * DML 명령어로 조작이 불가능한 경우가 많음!
    ...
    2) 뷰에 정의되어 있지 않고 테이블에 NOT NULL 제약 조건이 설정되어 잇는 경우 
    3) 산술연산식 또는 함수식으로 정의되어 있는 경우 
    4) DISTINCT 구문이 포함되어 있는 경우 
    5) JOIN을 이용하여 여러 테이블을 연결한 경우
    
    => 뷰는 대두분 조회의 용도로 사용한다. 따라서, 되도록이면 DML은 사용하지 말것
 ...
 */
 ...
 AS 
    ...
    FROM TT;
--> TT 라는 테이블이 존재하지 않기 떄문에 뷰도 생성 불가
...
--> 서브쿼리에 작성했던 조건에 부합되지 않기 때문에  변경 불가 (=> 오류 발생)
 ...
 UPDATE VW_EMP
   SET SALARY = 4000000
   WHERE EMP_ID = 204;
--> 서브커리에 작성했던 조건에 부합되기 때문에, 이 경우는 변경 가능
