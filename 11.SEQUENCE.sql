...
/*
   ...
   [표현법]
       ...
       [CYCLE | NOCYCLE]         --> 값의 순환여부 (기본값 NOCYCLE)
                                     * CYCLE : 시퀀스 값이 최대값에 도달하면 최소값으로 
                                                다시 순환하도록 설정
                                     * NOCYCLE : 시퀀스 값이 최대값에 도달하면 더이상
                                                 증가시키지 않도록 설정
    
       [NOCACHE | CACHE 숫자]     --> 캐시메모리 할당 여부 ( 기본값 CACHE 20)
                                    *캐시메모리 : 미리 발생될 값들을 생성해서 저장해두는 공간
                                      매번 호출될 떄마다 새로 번호를 생성하는 것이 아니라
...
/*
    * 시퀀스 사용 *
    ...
    - 시퀀스명.NEXTVAL : 시퀀스 값에 일정 값을 증가시켜 발생한 결과값.
                        현재 시퀀스 값에서 INCREMNET BY에 설정된 값만큼 증가된 값.
*/

-- * SEQ_EMPNO 시퀀스의 현재 시퀀스 값 조회
SELECT SEQ_EMPNO.CURRVAL FROM DUAL;
--> 오류발생!! NEXTVAL를 한번도 수행하지 않은 시퀀스는 CURRVAL를 사용할수 없음!
...
-- * 시퀀스 생성 (SEQ_ID) -> 300번부터 시작하도록, 캐시메모리 사용하지 않음, 1씩 증가.
...
INSERT INTO EMPLOYEE (EMP_ID, EMP_NAME, EMP_NO, JOB_CODE, HIRE_DATE)
    VALUES (SEQ_ID.NEXTVAL, '강파치', '250311-123456', 'J5', SYSDATE);

SELECT * FROM EMPLOYEE;

INSERT INTO EMPLOYEE (EMP_ID, EMP_NAME, EMP_NO, JOB_CODE, HIRE_DATE)
    VALUES (SEQ_ID.NEXTVAL, '남파치', '250311-123456', 'J5', SYSDATE);
