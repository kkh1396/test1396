...
/*
    * 인덱스 (INDEX)
       ...
       - 단점 : 인덱스를 위한 저장 공간이 필요.
                데이터 변경 작업(DML) 시 인덱스도 함께 업데이트가 되어 오히려 성능이 저하될수 있음
*/
...
-- * USER_INDEX_DATA 테이블에 기본키 추가 (=> ID 컬럼)
ALTER TABLE USER_INDEX_DATA ADD CONSTRAINT PK_IDX_ID PRIMARY KEY (ID);

-- * USER_INDEX_DATA 테이블에 UNIQUE 추가 (=> EMAIL 컬럼)
ALTER TABLE USER_INDEX_DATA ADD CONSTRAINT UQ_IDX_EAMIL UNIQUE (EMAIL);
...
-- * 인덱스가 설정되지 않은 테이블 (USER_MOCK_DATA)
EXPLAIN PLAN FOR
